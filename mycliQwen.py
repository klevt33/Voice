...
chat_url = "https://chat.qwenlm.ai/"

# Specify the debugging address for the already opened Chrome browser
debugger_address = "localhost:9222"

# Define the filename with the initial prompt to send to DeepSeek
...
def send_to_chat(driver, prompt):
    try:
        # Find the prompt input field using CSS selector
        prompt_input = driver.find_element(By.CSS_SELECTOR, "textarea#chat-input")
...
@click.command()
@click.option("--model", default="medium", help="Model to use", type=click.Choice(["tiny","base", "small","medium","large","large-v2","large-v3","turbo"]))
...
def main(model: str, english: bool, verbose: bool, energy:  int, pause: float, dynamic_energy: bool, save_file: bool, device: str, loop: bool, dictate: bool,mic_index:Optional[int],list_devices: bool,faster: bool,hallucinate_threshold:int) -> None:
    ...
    # Open the prompt file and read its contents into a string variable
    try:
        with open(prompt_file, 'r') as file:
            prompt_content = file.read()
        send_to_chat(driver, prompt_content)
    except FileNotFoundError:
        print(f"Error: The file '{prompt_file}' was not found in the current directory.")
